...
class GigaChatAPI:
    ...
    async def get_embedding(self, text: str, model: str = 'Embeddings') -> list[float] | None:
        """Получение векторного представления (эмбеддинга) для текста."""
        if not self.access_token:
            logger.error("❌ Невозможно получить эмбеддинг: токен доступа не получен.")
            return None

        url = "https://gigachat.devices.sberbank.ru/api/v1/embeddings"
        payload = {
            "model": model,
            "input": [text] # API ожидает массив строк
        }
        headers = {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload, ssl=False) as response:
                    if response.status == 200:
                        ...
                        if embedding:
                            logger.info(f"✅ Эмбеддинг для текста '{text[:30]}...' успешно получен.")
                            return embedding
                        else:
                            logger.error(f"❌ Ответ API на эмбеддинги не содержит вектор. Ответ: {data}")
                            return None
                    else:
                        error_text = await response.text()
                        logger.error(f"❌ Ошибка API GigaChat (Embeddings): {response.status} - {error_text}")
                        return None
        except aiohttp.ClientError as e:
            logger.error(f"❌ Ошибка сети при получении эмбеддинга: {str(e)}")
            return None
        except Exception as e:
